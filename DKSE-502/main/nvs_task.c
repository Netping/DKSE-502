...
esp_err_t save_data_blok(void) {
	esp_err_t err=0;
		if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
				...
				ESP_LOGE("NVS", "Error OUTPFF MEMORY");
	...
	if (err != ESP_OK) {
				ESP_LOGW("OUT_SAVE", "Error %X save data to flash", err);
				err=0;
			}


//	err = err
//			| nvs_set_blob(nvs_data_handle, get_name(out_name_1),
//					OUT_PORT[0].name, 16);
//
//
//	err = err
//				| nvs_set_blob(nvs_data_handle, get_name(out_name_2),
//						OUT_PORT[1].name, 16);
//	err = err
//				| nvs_set_blob(nvs_data_handle, get_name(out_name_3),
//						OUT_PORT[2].name, 16);
...
//	lens = 16;
//		err = err
//				| nvs_get_blob(nvs_data_handle, get_name(out_name_1),
//						OUT_PORT[0].name, &lens);
//
//		err = err
//						| nvs_get_blob(nvs_data_handle, get_name(out_name_2),
//								OUT_PORT[1].name, &lens);
//		err = err
//						| nvs_get_blob(nvs_data_handle, get_name(out_name_3),
//								OUT_PORT[2].name, &lens);
...
//     HTTPD_SERVER_PORT =FW_data.V_WEB_PORT;
	...
	memcpy((uint32_t*) &FW_data.smtp.V_EMAIL_FROM,
			(uint32_t*) "dkst502@smtp.netping.ru",
			sizeof("dkst502@smtp.netping.ru"));
	...
	memcpy((uint8_t*) &FW_data.sys.V_Name_dev, (uint8_t*) "DKST502",(size_t) strlen("DKST502"));
	memcpy((uint8_t*) &FW_data.sys.V_CALL_DATA, (uint8_t*) "plcontroller.ru",(size_t) strlen("plcontroller.ru"));
