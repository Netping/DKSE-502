...
#endif
#if exp_port == 1
uint8_t PORT_O[int_port_n] = { P_O0, P_O1 };
uint8_t PORT_OE[out_port_n - int_port_n];
...
//		sprintf(out_small, "Старт модуля выходов\n\r");
		sprintf(out_small, "Start module output\n\r");
		break;
...
void output_port(void *pvParameters) {
	...
	for (uint8_t ct = 0; ct < (out_port_n - int_port_n); ct++) {
		PORT_OE[ct] = ct;
	}

	for (uint8_t ct = 0; ct < int_port_n; ct++) {
		//	FW_data.gpio.OUT_PORT[ct].delay = 3000;
		gpio_set_direction(PORT_O[ct], GPIO_MODE_OUTPUT);
		OUT_PORT[ct].S_gpio_port = xSemaphoreCreateMutex();
		xSemaphoreGive(OUT_PORT[ct].S_gpio_port);
	...
	for (uint8_t ct = int_port_n; ct < out_port_n; ct++) {
		OUT_PORT[ct].S_gpio_port = xSemaphoreCreateMutex();
		xSemaphoreGive(OUT_PORT[ct].S_gpio_port);
	}
		...
		for (uint8_t ct = int_port_n; ct < out_port_n; ct++) {
			...
			if (xSemaphoreTake(OUT_PORT[ct].S_gpio_port,
				...
				if (OUT_PORT[ct].aflag == 1) {
					...
					if (OUT_PORT[ct].type_logic == 3) {
						if (OUT_PORT[ct].sost == 0) {
							gpio_set_level_ext(PORT_OE[ct], 0);

							OUT_PORT[ct].realtime = 0;
							log_out_event_cntl(&(OUT_PORT[ct]), ct);
							vTaskDelay(OUT_PORT[ct].delay / portTICK_PERIOD_MS);
							gpio_set_level_ext(PORT_OE[ct], 1);
...
void get_name_nvs(char *in, char *out, uint8_t ct) {
	sprintf((char*) out, "%s%d", in, ct);
}
...
esp_err_t save_data_output(void) {
	...
	for (ct_s = 0; ct_s < out_port_n; ct_s++) {
		...
		sprintf((char*) name_line, "out_d%d", ct_s);
		err = err
		...
		memset((uint8_t*) name_line, 0, 32);
		sprintf((char*) name_line, "out_p%d", ct_s);
		err = err
				| nvs_set_u16(nvs_data_handle, (char*) name_line,
		...
		memset((uint8_t*) name_line, 0, 32);
		sprintf((char*) name_line, "out_t_log_%d", ct_s);
		err = err
				| nvs_set_u16(nvs_data_handle, (char*) name_line,
		...
		memset((uint8_t*) name_line, 0, 32);
		get_name_nvs(get_name(out_name_), (char*) name_line, ct_s);

		err = err
				| nvs_set_blob(nvs_data_handle, (char*) name_line,
						OUT_PORT[ct_s].name, 16);

		memset((uint8_t*) name_line, 0, 32);
		get_name_nvs(get_name(out_s_name_), (char*) name_line, ct_s);

		err = err
				| nvs_set_blob(nvs_data_handle, (char*) name_line,
						OUT_PORT[ct_s].set_name, 16);

		memset((uint8_t*) name_line, 0, 32);
		get_name_nvs(get_name(out_c_name_), (char*) name_line, ct_s);

		err = err
				| nvs_set_blob(nvs_data_handle, (char*) name_line,
						OUT_PORT[ct_s].clr_name, 16);
...
esp_err_t load_data_output(void) {
	...
	for (ct_s = 0; ct_s < out_port_n; ct_s++) {
		...
		if (err != ERR_OK) {
			ESP_LOGW("OUT_READ", "Error %s=%x read data to flash1-%d",
					(char* ) name_line, err, ct_s);
			err = 0;
		}
		...
		if (err != ERR_OK) {
			ESP_LOGW("OUT_READ", "Error %s=%x read data to flash2-%d",
					(char* ) name_line, err, ct_s);
			err = 0;
		}
		...
		err = err
				| nvs_get_u16(nvs_data_handle, (char*) name_line,
						&OUT_PORT[ct_s].type_logic);
		if (err != ERR_OK) {
			ESP_LOGW("OUT_READ", "Error %s=%x read data to flash3-%d",
					(char* ) name_line, err, ct_s);
			err = 0;
		}
		...
		err = err
				| nvs_get_blob(nvs_data_handle, (char*) name_line,
						(char*) &(OUT_PORT[ct_s].name), (size_t*) &lens);
		if (err != ERR_OK) {
			ESP_LOGW("OUT_READ", "Error %s=%x read data %d %s to flash4-%d\n\r",
					(char* ) name_line, err, lens, OUT_PORT[ct_s].name, ct_s);
			err = 0;
		}
			...
			err = err
					| nvs_get_blob(nvs_data_handle, (char*) name_line,
							(char*) &(OUT_PORT[ct_s].set_name), (size_t*) &lens);
			if (err != ERR_OK) {
				ESP_LOGW("OUT_READ",
						"Error %s=%x read data %d %s to flash5-%d\n\r",
						(char* ) name_line, err, lens, OUT_PORT[ct_s].set_name,
						ct_s);
				err = 0;
			}
				...
				err = err
						| nvs_get_blob(nvs_data_handle, (char*) name_line,
								(char*) &(OUT_PORT[ct_s].clr_name),
								(size_t*) &lens);
				if (err != ERR_OK) {
					ESP_LOGW("OUT_READ",
							"Error %s=%x read data %d %s to flash6-%d\n\r",
							(char* ) name_line, err, lens,
							OUT_PORT[ct_s].clr_name, ct_s);
					err = 0;
				}
...
//
//		memset((uint8_t*) &name_line, 0, 32);
//		sprintf((char*)name_line, "out_name_%d", ct_s);
//		printf( "%s\n\r", (char*)name_line);
//		err = err
//				| nvs_get_blob(nvs_data_handle, (char*) name_line,
//						&(OUT_PORT[ct_s].name), (size_t*)&lens);
//
//		if (err != ERR_OK) {
//			ESP_LOGW("OUT_READ", "Error %s=%x read data %d %s to flash4-%d\n\r",(char*) name_line,
//					err,lens,OUT_PORT[ct_s].name, ct_s);
//			err=0;
//		}
...
//
//		memset((uint8_t*) &name_line, 0, 32);
//		sprintf((char*)name_line, "out_s_name_%d", ct_s);
//		printf( "%s\n\r", (char*)name_line);
//		err = err
//				| nvs_get_blob(nvs_data_handle, (char*) name_line,
//						&(OUT_PORT[ct_s].set_name),  (size_t*)&lens);
//		if (err != ERR_OK) {
//			ESP_LOGW("OUT_READ", "Error %s=%x read data %d to flash5-%d\n\r", (char*)name_line,
//					err,lens, ct_s);
//			err=0;
//		}
//		lens = 16;
//
//		memset((uint8_t*) &name_line, 0, 32);
//		sprintf((char*)name_line, "out_c_name_%d", ct_s);
//		printf( "%s\n\r", (char*)name_line);
//		err = err
//				| nvs_get_blob(nvs_data_handle, (char*) name_line,
//						&(OUT_PORT[ct_s].clr_name), (size_t*)&lens);
//		if (err != ERR_OK) {
//			ESP_LOGW("OUT_READ", "Error %s=%x read data %d to flash6-%d\n\r",(char*) name_line,
//					err,lens, ct_s);
//			err=0;
//		}
				}
			...
			void load_def_output(void) {
				...
				for (ct_s = 0; ct_s < out_port_n; ct_s++) {
					...
					memcpy((uint8_t*) &OUT_PORT[ct_s].clr_name,
							(uint8_t*) "OFF", sizeof("OFF"));

					memset((uint8_t*) &OUT_PORT[ct_s].set_name, 0, 32);
					memcpy((uint8_t*) &OUT_PORT[ct_s].set_name, (uint8_t*) "ON",
							sizeof("ON"));
